...
const highAngleAdjust = 0.62;
const eyeHeight = 1.10;
const Gravity = 18.5;
const maxArrowSpeed = 53;
...
function shootArrow(entity,isHigh) {
    ...
    if (bow != null && arrow != null) {
        if(entity.name!=undefined) bot.log("[combat] shoot " + entity.name);
        else bot.log("[combat] shoot " + entity.username);
    ...
    function release() {
        if (entity != undefined) {
            ...
            var angle = (target.y - bot.entity.position.y) / distance;
            ...
            var discriminant = (ayg * ayg - Gravity * Gravity * ((x * x) + (z * z)));
            ...
            if (isNaN(heightAdjust)) heightAdjust = (distance * distance) * 0.003;
